package org.windwant.serverx.config;
...
import org.windwant.serverx.core.datasource.RoutingDataSource;
import org.windwant.serverx.core.mybatis.MapperScannerConfigurerProxy;
...
//    }
    ...
    @Order(value = 3)
    @Bean(name = "localDataProxy")
    public DataSourceProxy localDataProxy(@Qualifier("localDataSource") DruidDataSource localDataSource){
        return new DataSourceProxy(localDataSource);
    }

    @Order(value = 4)
    @Bean(name = "remoteDataProxy")
    public DataSourceProxy remoteDataProxy(@Qualifier("remoteDataSource") DruidDataSource remoteDataSource){
        return new DataSourceProxy(remoteDataSource);
    }

    @Bean(name = "routingDataSource")
    @Order(value = 5)
    public DataSource routingDataSource(@Qualifier("localDataProxy") DataSourceProxy localDataProxy,
                                        @Qualifier("remoteDataProxy") DataSourceProxy remoteDataProxy){
        RoutingDataSource routingDataSource = new RoutingDataSource();
        Map<Object, Object> dataSources = new HashMap<>();
        dataSources.put(org.windwant.serverx.core.mybatis.DataSource.Type.LOCAL.name(), localDataProxy);
        dataSources.put(org.windwant.serverx.core.mybatis.DataSource.Type.REMOTE.name(), remoteDataProxy);
        routingDataSource.setTargetDataSources(dataSources);
        routingDataSource.setDefaultTargetDataSource(localDataProxy);
    ...
    public MapperScannerConfigurer mapperScannerConfigurer() {
        ...
        mapperScannerConfigurerProxy.setBasePackage("org.windwant.serverx.mapper");
    ...
    public org.apache.ibatis.session.Configuration configuration(){
        ...
        config.getTypeAliasRegistry().registerAliases("org.windwant.serverx.model");
        config.getTypeAliasRegistry().registerAliases("org.windwant.common.api.model");
        //mybatis 自定义 typeHandler
        config.getTypeHandlerRegistry().register("org.windwant.serverx.core.mybatis.handler");
        return config;
    }
