...
public class WSProxyFrameHandler extends SimpleChannelInboundHandler<Object> {
    ...
    protected void channelRead0(ChannelHandlerContext context, Object webSocketFrame) throws Exception {
        if (webSocketFrame instanceof BinaryWebSocketFrame) {
            ...
            //channel维护管理 将每一个连接的channel 放入本地map缓存，并注册 k-v到consul
            String channelName = WSProxyChannelManager.mapRequestCodeToChannel(requestCode);
            if (WSProxyChannelManager.getConnectChannel(channelName) == null) {
                WSProxyChannelManager.registerConnectChannel(channelName, context.channel());
                ConsulUtil.putRequestChannel(NetUtil.getHost() + "_connect", requestCode);
            }
            //处理业务逻辑
            WSBusiHandler.dealBusi(context, dubboRequest);
        } else if(webSocketFrame instanceof FullHttpRequest) {//初始连接 http请求
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        logger.info("channel Active {}", new Date().getTime() + "_" + ctx.channel().id().asLongText());
    }
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        if(requestCode != null) {
            ...
            WSProxyChannelManager.removeChannel(WSProxyChannelManager.mapRequestCodeToChannel(requestCode), ctx.channel());
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        ...
        WSProxyChannelManager.removeChannel(WSProxyChannelManager.mapRequestCodeToChannel(requestCode), null);
